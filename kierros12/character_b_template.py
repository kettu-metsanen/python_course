...
class Character:
    ...
    def attack(self, target, weapon):
        ...
        if weapon not in WEAPONS:
            print("Attack fails: unknown weapon \"{}\".".format(weapon))
            return False
        
        if target.get_name() == self.__name:
            print("Attack fails:", self.__name ,"can't attack him/herself.")
            return False
        
        if weapon not in self.__bagaj:
            print("Attack fails: {} doesn't have \"{}\".".format(self.__name,weapon))
            return False
        
        target.set_hitpoints(WEAPONS[weapon])
        #print("Conan the Barbarian attacks Deadpool delivering 5 damage.")
        print(self.__name, "attacks", target.get_name(), "delivering", 
              WEAPONS[weapon], "damage.")
        if target.ret_hitpoints() <= 0:
            print("{} successfully defeats {}.".format(self.__name, target.get_name()))
        return True
