...
class Car:
    ...
    def print_information(self):
        ...
        print(f"The tank contains {self.__gas:.1f} liters of gas and the odometer shows {self.__odometr:.1f} kilometers.")
    ...
    def drive(self, distance):
        rashod = self.__consumption/100 * distance
        if rashod < self.__gas:
            self.__gas -= rashod
            self.__odometr += distance
        else:
            self.__odometr += self.__gas / (self.__consumption/100)
            self.__gas = 0
